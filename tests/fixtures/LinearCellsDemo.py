...
def main():
    ...
    for row in range(0, grid_row_dimensions):
        ...
        for col in range(0, grid_column_dimensions):
            ...
            # because the viewport Y axis points upwards, and we want to draw
            # the grid from top to down.
            viewport = (float(col) / grid_column_dimensions,
                        float(grid_row_dimensions - (row + 1)) /
                        grid_row_dimensions,
    ...
    if single_object:
        if vtk_version_ok(9, 0, 20210718):
            try:
                ...
                cam_orient_manipulator.SetParentRenderer(
                    renderers[single_object_key])
...
def make_triangle_strip():
    ...
    ug.SetPoints(points)
    ug.InsertNextCell(
        triangle_strip.GetCellType(),
        triangle_strip.GetPointIds())
...
def make_pentagonal_prism():
    ...
    ug.InsertNextCell(
        pentagonal_prism.GetCellType(),
        pentagonal_prism.GetPointIds())
...
def make_hexagonal_prism():
    ...
    ug.InsertNextCell(
        hexagonal_prism.GetCellType(),
        hexagonal_prism.GetPointIds())

    return ug


def make_tile(bounds, expansion_factor=0.5,
              thickness_ratio=0.05, shift_y=-0.05):
    '''
...
def vtk_version_ok(major, minor, build):
    ...
    needed_version = 10000000000 * \
        int(major) + 100000000 * int(minor) + int(build)
    try:
